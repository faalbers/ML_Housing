"""
    Training Data using Random Forrest regression
"""
...
def main():
    ...
    # lets train using Random Forest
    print("\nTesting Random Forrest Regression Model on partial training data...")
    forest_reg = RandomForestRegressor()
    forest_reg.fit(housing_prepared, housing_labels)
    showModelResults(forest_reg, housing_prepared, housing_labels, 5)
    showModelRMSE(forest_reg, housing_prepared, housing_labels)
    showModelRMSE_CrossVal(forest_reg, housing_prepared, housing_labels, 10)

    # Random Forrest Regressor hyperparameter grid search
    if False:
        param_grid = [
            {'bootstrap': [False], 'n_estimators': [40, 60, 80], 'max_features': [4, 5, 6]},
        ]

        param_grid = [
            {'bootstrap': [False], 'n_estimators': [60], 'max_features': [6]},
        ]
    ...
    # testing new Random Forest params
    print("\nTesting Random Forrest Regression Model on partial training data with new params ...")
    forest_reg = RandomForestRegressor(n_estimators=60, max_features=6)
    forest_reg.fit(housing_prepared, housing_labels)
    showModelResults(forest_reg, housing_prepared, housing_labels, 5)
    showModelRMSE(forest_reg, housing_prepared, housing_labels)
    showModelRMSE_CrossVal(forest_reg, housing_prepared, housing_labels, 10)
    ...
    for (importance, attribute) in sorted(zip(forest_reg.feature_importances_, housing_prepared_attribs), reverse=True):
